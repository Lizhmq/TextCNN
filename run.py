...
def main():
    ...
    data = getattr(utils, f"read_{options.dataset}")(options.data_path, options.train_name, options.valid_name, options.test_name)

    # data["vocab"] = sorted(list(set([w for sent in data["train_x"] + data["dev_x"] + data["test_x"] for w in sent])))
    # build vocab from training and valid set only.
